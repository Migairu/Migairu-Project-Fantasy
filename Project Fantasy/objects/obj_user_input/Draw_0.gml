...
var end_pos = string_length(user_text); // Ending position of the text to draw
...
if (text_width > text_area_width) {
    ...
    while (string_width(string_copy(user_text, start_pos, cursor_position - start_pos + 1)) > text_area_width && start_pos <= cursor_position) {
        start_pos++;
    }
}

// Adjust end_pos based on start_pos to ensure we only draw text that fits within the box
while (string_width(string_copy(user_text, start_pos, end_pos - start_pos + 1)) > text_area_width && end_pos >= start_pos) {
    end_pos--;
}

// Determine the text to draw based on adjusted start_pos and end_pos
var text_to_draw = string_copy(user_text, start_pos, end_pos - start_pos + 1);

if (cursor_visible && keyboard_active) {
    text_to_draw += "|"; // Add cursor for visual indication
}
text_to_draw += string_copy(user_text, cursor_position + 1, string_length(user_text) - cursor_position);
